...
function crud_mail_list()
{
    ...
    Crud::init("select {$settings['TABLE']}.{$settings['PREFIX']}_id, CONCAT(repeat_invoices_currency, ' ', repeat_invoices_cost) AS cost, {$settings['PREFIX']}_username, {$settings['PREFIX']}_status, services_name, {$settings['PREFIX']}_comment from {$settings['TABLE']} left join repeat_invoices on repeat_invoices_id={$settings['PREFIX']}_invoice and repeat_invoices_module='{$module}' left join services on services_id={$settings['TABLE']}.{$settings['PREFIX']}_type", $module)
        ->set_limit_custid_role('list_all')
        ->set_order($settings['PREFIX'].'_status', 'asc')
        ->set_title(_($settings['TITLE']).' '._('List'))
        ->enable_labels()
        ->set_labels([$settings['PREFIX'].'_id' => _('ID'),$settings['PREFIX'].'_username' => _('Username'), 'repeat_invoices_cost' => _('Cost'), $settings['PREFIX'].'_status' => _('Status'), 'services_name' => _('Package'), $settings['PREFIX'].'_comment' => _('Comment')])
        ->add_header_button(\MyAdmin\App::defaultTheme() == 'adminlte' ? \MyAdmin\App::link('index.php', 'choice=none.order_'.$settings['PREFIX']) : \MyAdmin\App::link('index.php', 'choice=none.buy_'.$settings['PREFIX']), _('Order'), 'primary', 'shopping-cart', _('Order').' '._($settings['TITLE']), 'client')
