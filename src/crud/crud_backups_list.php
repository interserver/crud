...
function crud_backups_list()
{
    ...
    Crud::init("select {$settings['TABLE']}.{$settings['PREFIX']}_id, {$settings['PREFIX']}_name, CONCAT(repeat_invoices_currency, ' ', ifnull(repeat_invoices_cost, 0.00)) as cost, {$settings['PREFIX']}_username, {$settings['PREFIX']}_status, services_name from {$settings['TABLE']} left join repeat_invoices on repeat_invoices_id={$settings['PREFIX']}_invoice and repeat_invoices_module='{$module}' left join {$settings['PREFIX']}_masters on {$settings['TABLE']}.{$settings['PREFIX']}_server={$settings['PREFIX']}_masters.{$settings['PREFIX']}_id left join services on services_id={$settings['TABLE']}.{$settings['PREFIX']}_type", $module)
        ...
        ->add_header_button(DEFAULT_THEME == 'adminlte' ? \MyAdmin\App::link('index.php', 'choice=none.order_storage') : \MyAdmin\App::link('index.php', 'choice=none.buy_storage'), _('Order'), 'primary', 'shopping-cart', _('Order Storage'), 'client')
