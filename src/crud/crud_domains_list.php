...
function crud_domains_list()
{
	...
	Crud::init("select concat('<a href=\"index.php?choice=none.view_domain&id=',{$settings['PREFIX']}_id,'\"><img src=\"https://shot.sh?w=300&h=100&img=',{$settings['PREFIX']}_hostname,'\"></a>') as screenshot,{$settings['PREFIX']}_id,{$settings['PREFIX']}_hostname,if({$settings['PREFIX']}_expire_date is null, '', if({$settings['PREFIX']}_expire_date = '0000-00-00 00:00:00','',{$settings['PREFIX']}_expire_date)) as {$settings['PREFIX']}_expire_date,{$settings['PREFIX']}_cost,{$settings['PREFIX']}_status from {$settings['TABLE']}", $module)
