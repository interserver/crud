...
function crud_dedicated_list()
{
    ...
    Crud::init($query, 'servers')
        ...
        ->enable_labels()
        ->set_labels($labels)
        ->add_header_button(\MyAdmin\App::link('index.php', 'choice=none.order_server'), _('Order'), 'primary', 'shopping-cart', _('Order Server'), 'client')
        ...
        ->disable_delete()
        ->disable_edit()
        ->add_row_button((\MyAdmin\App::ima() == 'admin' ? 'none.view_server_order&id=%id%' : 'none.view_server&id=%id%'), _('View Server'), 'primary', 'cog')
        ->go();
}
