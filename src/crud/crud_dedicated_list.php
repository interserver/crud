...
{
	Crud::init('select server_id, account_lid, primary_ipv4, server_hostname, server_status from servers left join accounts on account_id=server_custid left join assets on order_id=server_id', 'servers')
        ->set_limit_custid_role('list_all')
		->set_title(_('Dedicated List'))
		->set_order('server_id', 'desc')
		->enable_labels()
		->set_labels(['server_id' => _('ID'),'account_lid' => _('Client'), 'primary_ipv4' => 'IP', 'server_hostname' =>  _('Server Name'), 'server_status' => _('Status')])
