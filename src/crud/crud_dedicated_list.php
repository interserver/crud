...
function crud_dedicated_list()
{
    Crud::init('select server_id, account_lid, server_hostname, server_status from servers left join accounts on account_id=server_custid', 'servers')
        ...
        ->add_header_button($GLOBALS['tf']->default_theme == 'adminlte' ? $GLOBALS['tf']->link('index.php', 'choice=none.order_server') : $GLOBALS['tf']->link('index.php', 'choice=none.buy_server'), _('Order'), 'primary', 'shopping-cart', _('Order Server'), 'client')
