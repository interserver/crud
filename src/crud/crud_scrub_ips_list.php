...
/**
 * scrub ips List
 * @author Ganesh <gane.candoit@gmail.com>
 * @copyright 2025
 * @package MyAdmin
 * @category scrub_ips
 */
...
/**
 * Displays a list of all the scrub ips available to your current session
 *
 * @return void
 */
function crud_scrub_ips_list()
{
    $module = 'scrub_ips';
    ...
    Crud::init("select {$settings['TABLE']}.{$settings['PREFIX']}_id, CONCAT(repeat_invoices_currency, ' ', repeat_invoices_cost) AS cost, {$settings['PREFIX']}_ip, {$settings['PREFIX']}_status, services_name from {$settings['TABLE']} left join repeat_invoices on repeat_invoices_id={$settings['PREFIX']}_invoice and repeat_invoices_module='{$module}' left join services on services_id={$settings['TABLE']}.{$settings['PREFIX']}_type", $module)
        ...
        ->add_header_button(\MyAdmin\App::defaultTheme() == 'adminlte' ? \MyAdmin\App::link('index.php', 'choice=none.order_scrub_ip') : \MyAdmin\App::link('index.php', 'choice=none.buy_scrub_ip'), _('Deploy'), 'primary', 'cloud-upload-alt', _('Deploy').' '._($settings['TITLE']), 'client')
